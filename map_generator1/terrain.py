...
import heapq
import noise
# 3.5 lib not important # import cPickle
import gzip
import itertools
...
class MapGrid(object):
    ...
    def calc_edges(self):
        n = self.nvxs
        self.edge = np.zeros(n, bool) # np.bool
        for u in range(n):
            adjs = self.adj_vxs[u]
            if -1 in adjs:# or \
    ...
    def infill(self):
        ...
        while True:
            ...
            if np.all(sinks == -1):
                ...
                return
            #if tries == 1:
                # broken print("Infilling", np.sum(sinks != -1), np.mean(self.vxs[sinks != -1, :], 0),
...
#         ax.gca().add_collection(landpatchcol)
        ...
        plt.savefig(filename, dpi=300, **kwargs)
        plt.close()
...
if __name__ == '__main__':
    for i in range(2):
        ...
        while True:
            try:
                m = MapGrid('shore', 25000)
                filename = "./%s-%02d.png" % (m.mode, i)
                m.plot(filename)
                break
